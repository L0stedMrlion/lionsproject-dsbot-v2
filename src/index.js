...
const { Client, GatewayIntentBits, ActivityType, EmbedBuilder, Events, messageLink} = require("discord.js");
dotenv.config();
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.commandName === 'status') {
    const status_embd = new EmbedBuilder()
      .setTitle('📊 Status')
      .setDescription(`
    ...
    interaction.reply({ embeds: [status_embd] });
  }
});
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.commandName === 'joke') {
    ...
    const joke_embd = new EmbedBuilder();
    ...
    interaction.reply({ embeds: [joke_embd] });
  }
});
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.commandName === 'authentication') {
    ...
    interaction.reply({ content: "✉️ Message was send to user!", ephemeral: true });
